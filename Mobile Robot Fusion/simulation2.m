...
GPSRate = 0.01; % Hz
Ratio = (1/Ts)/GPSRate;
Gps_accu = 3; %Accuracy (meters)
Vel_accu = 0.1; % Accuracy (m)
...
Rn = 1*diag([Gps_accu^2 Gps_accu^2 Vel_accu^2 Vel_accu^2 0.001]);
...
YK_ODO = zeros(iterations,3); %hard
YK_GPS = [];
YK_Kalman = [];
...
%     ykinput = [yk_gps(1) yk_gps(2) yk(3)]'; 
    ykinput = [yk(1) yk(3) yk(5)]; % true value
...
%     Robot Kalman
    ...
    yk_measurement = [gps_x v_gps(1) gps_y v_gps(2) yk(5)]'
